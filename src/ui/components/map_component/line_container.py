...
class LineContainer(QWidget):
    ...
    def _update_geometry(self) -> None:
        ...
        # Add margin
        margin = max(int(self.WIDGET_MARGIN * self.geometry.scale), self.MIN_WIDGET_MARGIN)
        self.setGeometry(
            final_min_x - margin,
            final_min_y - margin,
    ...
    def update_label_style(self) -> None:
        ...
        font_size = max(int(self.LABEL_FONT_SIZE_BASE * self.geometry.scale), self.MIN_LABEL_FONT_SIZE)
    ...
    def _generate_control_points(self) -> None:
        ...
        for i, point in enumerate(self.geometry.scaled_points):
            control_point = {
                "pos": point,
                "index": i,
                "radius": max(int(self.BASE_CONTROL_POINT_RADIUS * self.geometry.scale), self.MIN_CONTROL_POINT_RADIUS),
            }
    ...
    def paintEvent(self, event):
        ...
        if self.edit_mode and self.control_points:
            self.renderer.draw_control_points(painter, self.control_points, widget_offset)
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == Qt.MouseButton.LeftButton and self.edit_mode:
            ...
            hit_index = self.hit_tester.test_control_point(event.pos(), self.control_points, widget_offset)
            if hit_index >= 0:
    ...
    def mouseMoveEvent(self, event):
        ...
        if self.dragging_control_point and self.dragged_point_index >= 0:
            ...
            self.geometry.update_point(self.dragged_point_index, (original_x, original_y))

            # Update widget geometry and trigger repaint
            self._update_geometry()
            self.update()

            print(f"Dragging point {self.dragged_point_index} to ({original_x}, {original_y})")
            event.accept()
        else:
    ...
    def _insert_point_at_segment(self, segment_index: int, insertion_point: QPoint) -> None:
        ...
        print(f"Inserting point at segment {segment_index}: ({original_x}, {original_y})")
