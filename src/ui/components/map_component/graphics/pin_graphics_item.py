...
class PinGraphicsItem(QGraphicsItem):
    ...
    def mouseReleaseEvent(self, event) -> None:
        ...
        if event.button() == Qt.MouseButton.LeftButton and self.dragging:
            ...
            self._emit_position_changed(scene_pos.x(), scene_pos.y())
    ...
    def get_original_coordinates(self) -> tuple[float, float]:
            ...
            while parent_widget:
                if hasattr(parent_widget, "graphics_adapter"):
                    if hasattr(parent_widget.graphics_adapter, "signal_bridge"):
                        parent_widget.graphics_adapter.signal_bridge.pin_clicked.emit(
                            self.target_node
                        )
                        break
                        ...
                        if hasattr(
                            ...
                            parent_widget.graphics_adapter.feature_manager.signal_bridge.pin_clicked.emit(
                                self.target_node
                            )
                            break
    ...
    def _emit_position_changed(self, x: float, y: float) -> None:
        """Emit position changed signal.
        ...
        if scene and hasattr(scene, "feature_items"):
            ...
            while parent_widget:
                if hasattr(parent_widget, "graphics_adapter"):
                    if hasattr(parent_widget.graphics_adapter, "signal_bridge"):
                        parent_widget.graphics_adapter.signal_bridge.pin_moved.emit(
                        ...
                        if hasattr(
                            ...
                            parent_widget.graphics_adapter.feature_manager.signal_bridge.pin_moved.emit(
                                self.target_node, x, y
                            )
                            break
                parent_widget = getattr(parent_widget, "parent", lambda: None)()
