...
class GraphicsCoordinateMapper:
    ...
    def scene_to_original(self, scene_point: QPointF) -> Tuple[int, int]:
        ...
        x = scene_point.x() / self.scale_factor
        y = scene_point.y() / self.scale_factor
    ...
    def get_scene_bounds(self) -> QRectF:
        ...
        return QRectF(
            0,
            0,
            self.scene_width * self.scale_factor,
    ...
    def scale_points(
        self, points: List[QPointF], scale: float, origin: QPointF = QPointF(0, 0)
    ) -> List[QPointF]:
