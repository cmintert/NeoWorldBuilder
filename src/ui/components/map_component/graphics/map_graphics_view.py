...
class MapGraphicsView(QGraphicsView):
    ...
    def mousePressEvent(self, event: QMouseEvent) -> None:
        ...
        if event.button() == Qt.MouseButton.MiddleButton:
            ...
            if self.scene():
                ...
                # Convert to integers for the signal (for compatibility)
                self.click_at_coordinates.emit(int(orig_x), int(orig_y))
    ...
    def paintEvent(self, event) -> None:
        ...
        if self._drawing_manager and hasattr(self._drawing_manager, "is_drawing_line"):
            if self._drawing_manager.is_drawing_line:
                ...
                hasattr(self._drawing_manager, "is_drawing_branching_line")
                and self._drawing_manager.is_drawing_branching_line
            ):
                logger.debug("Drawing temporary line - branching line mode")
                self._draw_temporary_line()
            elif (
                hasattr(self._drawing_manager, "is_drawing_polygon")
                and self._drawing_manager.is_drawing_polygon
            ):
                logger.debug("Drawing temporary polygon")
                self._draw_temporary_polygon()
    ...
    def _draw_temporary_line(self) -> None:
        ...
        logger.debug(
            f"Drawing temporary line with {len(temp_points)} points: {temp_points[:3]}..."
        )
        ...
        painter.end()
        logger.debug(
            f"Drew temporary line with {len(view_points)} points (start/end indicators included)"
        )

    def _draw_temporary_polygon(self) -> None:
        """Draw the temporary polygon preview on the viewport."""
        if not self._drawing_manager:
            return

        # Get temporary points from drawing manager
        temp_points = getattr(self._drawing_manager, "current_polygon_points", [])

        if len(temp_points) < 1:
            logger.debug(f"No points for polygon preview: {len(temp_points)}")
            return

        logger.debug(f"Drawing temporary polygon with {len(temp_points)} points")
        ...
        for point in temp_points:
            if self.scene():
                ...
                scene_point = self.scene().original_to_scene_coords(point[0], point[1])
                view_point = self.mapFromScene(scene_point)
                # Convert QPoint to QPointF for polygon drawing
                view_points.append(QPointF(view_point))

        if len(view_points) >= 3:
            # Draw filled polygon preview (semi-transparent)
            from PyQt6.QtGui import QPolygonF

            polygon = QPolygonF(view_points)

            # Set up fill brush (semi-transparent blue)
            fill_color = QColor("#4a90e2")
            fill_color.setAlpha(80)  # Semi-transparent
            painter.setBrush(QBrush(fill_color))

            # Set up outline pen (dashed blue)
            pen = QPen(QColor("#3388FF"), 2, Qt.PenStyle.DashLine)
            painter.setPen(pen)
        ...
        # Draw vertex markers
        if len(view_points) >= 1:
            # First point - green circle
            painter.setBrush(QColor(0, 255, 0, 180))
            painter.setPen(QPen(QColor(0, 150, 0), 2))
            painter.drawEllipse(view_points[0], 8, 8)

            # Other points - red circles
            if len(view_points) > 1:
        ...
        painter.end()
        logger.debug(f"Drew temporary polygon with {len(view_points)} vertices")
    ...
    def set_cursor_for_mode(self, mode: str) -> None:
        ...
        Args:
            mode: One of 'default', 'pin_placement', 'line_drawing',
                  'branching_line_drawing', 'polygon_drawing', 'edit', 'crosshair', 'pointing'
        """
        ...
        cursor_map = {
            ...
            "polygon_drawing": Qt.CursorShape.CrossCursor,
        ...
        logger.info(
            f"MapGraphicsView: Set Qt built-in cursor {cursor_shape} for mode: {mode}"
        )
