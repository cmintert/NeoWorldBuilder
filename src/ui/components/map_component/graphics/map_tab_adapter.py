...
class MapTabGraphicsAdapter(QObject):
        ...
        if layout:
            ...
            for i in range(layout.count()):
                ...
                if item and item.widget():
                    ...
                    if hasattr(widget, "widget") and hasattr(
                        widget, "setWidget"
                    ):  # QScrollArea
    ...
    def _setup_signal_forwarding(self) -> None:
        ...
        self.graphics_view.coordinates_changed.connect(self._handle_coordinates_changed)
        self.graphics_view.key_press_event.connect(self._handle_key_press_event)
        # Forward zoom requests from graphics view to map tab
        self.graphics_view.zoom_requested.connect(self._handle_zoom_requested)
        ...
        self.feature_manager.signal_bridge.line_clicked.connect(
            lambda node: self.pin_clicked.emit(
                node
    ...
    def _handle_click_at_coordinates(self, x: int, y: int) -> None:
        ...
        if hasattr(self.map_tab, "mode_manager"):
            ...
            if mode_manager.pin_placement_active:
                ...
                if hasattr(self.map_tab, "event_handler"):
                    self.map_tab.event_handler.handle_coordinate_click(x, y)
            elif mode_manager.polygon_drawing_active:
                # Polygon drawing mode - forward to event handler
                if hasattr(self.map_tab, "event_handler"):
                    self.map_tab.event_handler.handle_coordinate_click(x, y)
    ...
    def _handle_zoom_requested(self, zoom_level: float) -> None:
        ...
        if hasattr(self.map_tab, "toolbar_manager") and hasattr(
            self.map_tab.toolbar_manager, "zoom_slider"
        ):
    ...
    def _connect_drawing_manager(self) -> None:
        ...
        try:
            if hasattr(self.map_tab, "drawing_manager"):
                ...
                logger.debug(
                    f"Connected drawing manager to graphics view: {type(drawing_manager)}"
                )
            else:
    ...
    def _handle_line_geometry_changed(
        self, node_name: str, geometry_data: list
    ) -> None:
    ...
    def enable_migration(self) -> None:
        ...
        if not self.is_migrated:
            ...
            if hasattr(self.map_tab, "drawing_manager") and hasattr(
                self.graphics_view, "_drawing_manager"
            ):
                logger.info(
                    f"Drawing manager connection verified: graphics view has drawing manager: {self.graphics_view._drawing_manager is not None}"
                )
            else:
    ...
    def _migrate_existing_features(self) -> None:
        ...
        try:
            ...
            if hasattr(self.map_tab, "feature_manager"):
                ...
                logger.info(
                    f"Migrating {len(existing_features)} existing features to graphics system"
                )
                        ...
                        if hasattr(widget_feature, "pin_svg"):  # Pin feature
                            ...
                            if hasattr(line_geometry, "branches"):
                                if line_geometry.is_branching:
                                    ...
                                    branches = [
                                        branch.copy()
                                        for branch in line_geometry.branches
                                    ]
                                    graphics_manager.add_branching_line_feature(
                                        node_name, branches
                                    )
                                    logger.debug(
                                        f"Migrated branching line: {node_name}"
                                    )
                                else:
                                    # Simple line
                                    points = (
                                        line_geometry.branches[0].copy()
                                        if line_geometry.branches
                                        else []
                                    )
