...
class GraphicsFeatureManager(QObject):
            ...
            if hasattr(parent, "parent") and hasattr(parent.parent(), "config"):
                ...
                return {
                    "map": {
                        ...
                        "BASE_PIN_HEIGHT": getattr(
                            config_obj.map, "BASE_PIN_HEIGHT", 32
                        ),
    ...
    def add_pin_feature(
        ...
        pin_item = PinGraphicsItem(
            target_node=node_name, x=x, y=y, config=self._get_config_dict()
        )
    ...
    def add_line_feature(
        ...
        logger.debug(
            f"Creating line graphics item for {node_name} with style properties: {properties}"
        )
    ...
    def add_branching_line_feature(
        self,
        node_name: str,
        branches: List[List[Tuple[int, int]]],
        properties: Optional[Dict[str, Any]] = None,
        branch_assignments: Optional[Dict[str, str]] = None,
    ) -> Any:
        ...
        # If branch assignments are provided, load them into the geometry
        if branch_assignments:
            # Replace the UnifiedLineGeometry with BranchingLineGeometry to support stable IDs
            branching_geometry = BranchingLineGeometry(branches)
            branching_geometry.load_branch_assignments_from_flat_properties(
                branch_assignments
            )

            # Update the line item's geometry with the one containing stable IDs
            line_item.geometry = branching_geometry
        ...
        logger.info(
            f"Created branching line feature: {node_name} with {len(branches)} branches and {len(branch_assignments or {})} assignments"
        )
    ...
    def update_feature_geometry(
        self, node_name: str, geometry: List[Tuple[int, int]]
    ) -> None:
