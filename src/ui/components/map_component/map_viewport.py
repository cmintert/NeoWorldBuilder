...
from PyQt6.QtGui import QMouseEvent, QWheelEvent, QKeyEvent, QCursor, QPainter, QPen, QColor
...
class MapViewport(QLabel):
    ...
    def _get_original_coordinates(self, widget_pos: QPoint) -> Optional[Tuple[int, int]]:
        ...
        if not (0 <= scaled_x <= pixmap_size.width() and 0 <= scaled_y <= pixmap_size.height()):
            return None
    ...
    def _is_in_special_mode(self) -> bool:
        ...
        return (getattr(self.parent_map_tab, 'pin_placement_active', False) or
                getattr(self.parent_map_tab, 'line_drawing_active', False) or
                getattr(self.parent_map_tab, 'edit_mode_active', False))
